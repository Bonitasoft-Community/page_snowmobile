...
public class GeneratorSql {
  ...
  public void sqlCreateColumn(final BdmField bdmField, final boolean isComment) {
    operationStatus.addSqlUpdate("alter table " + bdmField.getBusinessObject().getSqlTableName() + " add " + getSqlCreate(bdmField) + ";", isComment);
  ...
  public void sqlColumnLengthChange(final BdmField bdmField) {
    ...
    operationStatus.addSqlUpdate("alter table " + getSqlTableName(bdmField) + " alter column " + getSqlCreate(bdmField) + ";", false);
  }

  public void sqlColumnTypeChange(final BdmField bdmField) {
    // try to alter it
    operationStatus.addSqlUpdate("alter table " + getSqlTableName(bdmField) + " alter column " + getSqlCreate(bdmField) + ";",
  ...
  public void sqlIsForeignKey(final BdmField bdmField) {
    ...
    operationStatus.addSqlUpdate("alter table " + bdmField.getBusinessObject().getSqlTableName() + " create column " + getSqlCreate(bdmField) + ";", false);
  ...
  public void sqlIsNotForeignKey(final BdmField bdmField, final JdbcColumn jdbcColumn) {
    ...
    operationStatus.addSqlUpdate("alter table " + bdmField.getBusinessObject().getSqlTableName() + " drop column " + jdbcColumn.getColName() + ";", false);
  }
  ...
  public CollectionAttributes sqlCreateCollection(final BdmField bdmField) {
    ...
    if (bdmField.isAggregation()) {
      ...
      script.append(" add constraints fk_" + collectionAttributes.columValue + " foreign key(" + collectionAttributes.columValue + ") reference "
  ...
  public void sqlDropIndex(final String tableName, final String indexName) {
    operationStatus.addSqlUpdate("drop index " + indexName.toLowerCase() + " on " + tableName + ";", parametersCalcul.commentDropIndex);
  }
  ...
  public void sqlCreateForeignConstraint(final BdmField bdmField, final boolean isComment) {
    ...
    script.append(" add constraints fk_" + bdmField.getSqlColName() + " foreign key(" + bdmField.getSqlColName() + ") reference "
        + bdmField.getSqlReferenceTable() + " (" + cstColumnPersistenceId + ");");
  ...
  public class ConversionItem {
    ...
    public ConversionItem(final String btmType, final String jdbcSqlType, final int jdbcType, final boolean lengthIsImportant) {
      bdmType = btmType;
    ...
    public ConversionItem(final String btmType, final String jdbcSqlType, final int jdbcType, final boolean lengthIsImportant, int overrideLength) {
      bdmType = btmType;
      this.jdbcSqlType = jdbcSqlType;
      this.jdbcType = jdbcType;
      this.lengthIsImportant = lengthIsImportant;
  ...
  private final ConversionItem[] postgresConversion = new ConversionItem[] {
      ...
      new ConversionItem("OFFSETDATETIME", "varying", java.sql.Types.VARCHAR, true, 30),
      new ConversionItem("LOCALDATE", "varying", java.sql.Types.VARCHAR, true, 10),
      new ConversionItem("LOCALDATETIME", "varying", java.sql.Types.VARCHAR, true, 30),
  ...
  private final ConversionItem[] h2Conversion = new ConversionItem[] {
      ...
      new ConversionItem("OFFSETDATETIME", "varchar", java.sql.Types.VARCHAR, true, 30),
      new ConversionItem("LOCALDATE", "varchar", java.sql.Types.VARCHAR, true, 10),
      new ConversionItem("LOCALDATETIME", "varchar", java.sql.Types.VARCHAR, true, 30),
  ...
  public String getSqlType(final BdmField bdmField) {
    ...
    operationStatus.addErrorMsg("Type unknow [" + bdmField.fieldType + "] BusinessObject[" + bdmField.getBusinessObject().getName() + "] field[" + bdmField.getName()
        + "]");
    return "varchar(10)";
  }
  ...
  public String getSqlType(final JdbcColumn jdbcColumn) {
    ...
    operationStatus.addErrorMsg("JdbcType unknow [" + jdbcColumn.dataType + "] JdbcTable[" + jdbcColumn.getJdbcTable().getTableName() + "] field["
