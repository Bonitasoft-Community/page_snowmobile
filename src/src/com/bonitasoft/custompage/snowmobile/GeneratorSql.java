...
public class GeneratorSql {
    ...
    protected void sqlGenerateTable(final BdmBusinessObject bdmContent)
    {
        ...
        sqlCreate.append("  constraint " + bdmContent.tableName + "_pkey primary key(" + cstColumnPersistenceId + ")\n");
    ...
    public void sqlIsNotForeignKey(final BdmField bdmField, final JdbcColumn jdbcColumn)
    {
        ...
        operationStatus.addSqlUpdate("alter table " + bdmField.getBusinessObject().getSqlTableName() + " drop column " + jdbcColumn.colName + ";", false);
    }
    ...
    public String getSqlType(final BdmField bdmField)
    {
        ...
        if (oneConversion != null) {
            return oneConversion.jdbcSqlType + (oneConversion.lengthIsImportant ? "(" + bdmField.fieldLength + ")" : "");
        }
        operationStatus.addErrorMsg("Type unknow [" + bdmField.fieldType + "] BusinessObject[" + bdmField.getBusinessObject().name + "] field[" + bdmField.name
                + "]");
        return "varchar(10)";
    }
    ...
    public String getSqlType(final JdbcColumn jdbcColumn)
    {
        ...
        for (final ConversionItem oneConversion : conversionTable)
        {
            if (oneConversion.jdbcType == jdbcColumn.dataType) {
                return oneConversion.jdbcSqlType + (oneConversion.lengthIsImportant ? "(" + jdbcColumn.length + ")" : "");
            }
        }
