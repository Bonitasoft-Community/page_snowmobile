    ...
    public void calculSqlScript(final ParametersCalcul parametersCalcul, final OperationStatus operationStatus) {
            ...
            for (final BdmField bdmField : bdmBusinessObject.getListFields()) {
                ...
                if (bdmField.isCollection()) {
                    if (bdmField.isComposition()) {
                        ...
                        continue;
                    } else if (jdbcTable.getCollectionTable(bdmBusinessObject.getSqlTableName(), bdmField.name, parametersCalcul) != null) {
        ...
        for (final BdmBusinessObject bdmBusinessObject : bdmContent.getListBdmBusinessObject().values()) {
            ...
            for (final JdbcColumn jdbcColumn : mapColums.values()) {
                if (jdbcColumn.colName.endsWith(GeneratorSql.cstSuffixColumnOrder) && !mapBdmIsReferencedInFather.contains(jdbcColumn.colName)) {
            ...
            if (jdbcTable != null) {
                for (final JdbcColumn jdbcColumn : jdbcTable.getListColumns()) {
                    if (bdmBusinessObject.getFieldBySqlColumnName(jdbcColumn.colName) == null && !listOfCompositionField.contains(jdbcColumn.colName)) {
                        ...
                        generatorSql.sqlDropColumn(jdbcTable.getTableName(), jdbcColumn.colName, parametersCalcul.commentDropColumn);
                    }
                }
            }
